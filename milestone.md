...
| 이미 존재 | 위치 |
|---|---|
| Stock-Agent MCP skill (`StockSkill`) | [gateway/yunam/mcp/stock.py](gateway/yunam/mcp/stock.py) |
...
- [mcp/stock.py](gateway/yunam/mcp/stock.py)에 `StockMCPClient` + `StockSkill` 구현
- `YUNAM_STOCK_MCP_URL` 환경변수로 SSE 연결, 도구는 MCP `list_tools`로 동적 발견 (정렬됨 → 캐시 순서 안정)
- 현재 declared scope: `Scope.KNOWLEDGE` ← **여기는 점검 필요**. CLAUDE.md governance 원칙상 *분리된 scope*가 좋습니다 (`finance:supply_read` 등). Phase 2.3에서 처리.
- → **재구현 없음**. Phase 2.3의 Finance Guardrail 사용자가 Stock-Agent를 *소비*하는 sub-agent로 설계.
...
```
*기존 stock MCP의 scope는 현재 `Scope.KNOWLEDGE` → **이 기회에 `Scope.STOCK_SUPPLY_READ`로 분리**하고 governance 일관성 회복.*
...
6. **Stock MCP scope 분리**: 현 `KNOWLEDGE` → `STOCK_SUPPLY_READ`로 바꾸는 작업을 Phase 2.3에 포함해도 되는지? (별도 governance 정비 PR로 분리할지)

이 6개에 답이 오면 → Phase 2.0 audit + Usage Tracking부터 PR 단위로 착수합니다.
